...
- 구글 쇼핑 데이터에는 판매처 상품 주소가 없고, 주소를 알려주는 조회 토큰은 하루이틀이면 만료됩니다. 그래서 수집한 지 하루 안 상품만 첫 클릭 때 조회해 저장하고, 나머지는 그 판매처 사이트의 검색 결과로 보냅니다. 이 방식이 가능하도록 검색 주소를 직접 확인한 판매처 12곳만 수집합니다.
