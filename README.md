...
- **패션 상품 검색** — 구글 쇼핑에서 매일 수집하고 AI로 태깅한 상품 DB에서 찾고, 재랭킹·동의어 보정으로 관련도를 높입니다. 기본은 무신사 상품이고 "다른 데서도 보여줘"라고 하면 29CM·KREAM·브랜드 공식몰 등 지원 판매처까지 섞어 보여줍니다.
- **매번 다른 추천** — 이미 본 상품을 브라우저에 기억해 두고 뒤로 보내, 대화를 지우고 같은 질문을 해도 새 상품을 보여줍니다.
...
- **상품 미리보기** — 카드를 누르면 AI 태그, 프로필·찜 기준 추천 이유, 비슷한 상품, "이런 스타일 더 찾아줘"를 보여주고, 구매 버튼은 그 상품의 판매처(상품 페이지나 판매처 검색 결과)로 이동합니다.
...
| 영속화      | localStorage (`sosie:profile` / `sosie:messages` / `sosie:favorites` / `sosie:language` / `sosie:seen-products`) |
| 다크 모드   | next-themes                                     |
| 테스트      | Vitest                                          |
| 호스팅      | Vercel                                          |
...
- 네이버 쇼핑 검색 API가 2026-07-31에 종료되어, 실시간 검색 대신 매일 수집하는 구조로 바꿨습니다. 구글 쇼핑 실시간 호출은 20~57초가 걸려 채팅에 쓸 수 없었고, 수집 단계에서 Gemini가 브랜드·품목·스타일을 태깅해 오히려 스타일 매칭이 정확해졌습니다. 대안 검토와 실측은 `.claude/rules/product/catalog.md`에 있습니다.
- 구글 쇼핑 데이터에는 판매처 상품 주소가 없고, 주소를 알려주는 조회 토큰은 하루이틀이면 만료됩니다. 그래서 첫 클릭 때 구글 웹검색으로 판매처 상품 상세 페이지를 찾아 저장합니다. 구글 결과 제목이 잘려 같은 브랜드의 다른 상품이나 다른 색상으로 가는 문제가 있어, 제목과 요약의 상품명 일치도와 모델명 일치를 따져 확신이 없으면 그 판매처 검색 결과로 보냅니다. 검색 주소를 직접 확인한 판매처 12곳만 수집합니다.
- AI가 "다른 컬러도" 같은 후속 요청에서 품목을 빠뜨리거나 반복된 상품을 새 상품처럼 소개하는 문제는 프롬프트만으로 막히지 않아, 서버가 직전 품목을 이어 붙이고 검색 결과에 반복 안내 문구를 넣어 해결했습니다.
- 프로필이 있으면 같은 검색 10번에 서로 다른 상품이 16개뿐이었습니다. 이미 본 상품을 뒤로 보내고 후보 풀을 넓혀 반복을 없애되, 선호 스타일·브랜드 상품이 적은 품목에서는 이미 본 선호 상품을 최대 2개까지 다시 보여줘 취향 반영이 사라지지 않게 균형을 맞췄습니다.
...
- `SERPAPI_API_KEY` — 구글 쇼핑 수집과 구매 링크 상품 페이지 찾기 (https://serpapi.com, 카드 미등록 무료 플랜)
