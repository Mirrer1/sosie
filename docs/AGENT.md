...
**프로필 주입**: [`src/app/api/chat/route.ts`](../src/app/api/chat/route.ts)의 `formatProfile()`가 한 줄 텍스트로 변환해 system prompt 끝에 append. 클라이언트는 매 요청 `useChat`의 `body` 옵션으로 프로필 + 자주 찜한 브랜드(`favoriteBrands`)를 함께 전송하고, 라우트는 `formatFavoriteBrands()`로 찜 브랜드도 프롬프트에 덧붙임.
