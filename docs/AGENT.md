...
```ts
{
  ...
  brands?: string[]
  size?: 'XS' | 'S' | 'M' | 'L' | 'XL'
  budget?: { min?: number; max?: number }   // 사용자가 말한 금액 그대로 (단위 원)
...
**budget 정책**:
- AI는 사용자가 말한 금액을 그대로 min/max에 담음 (하한만/상한만/둘 다/제한 없음)
- 예: "20만원까지" → `{ max: 200000 }`, "10만~30만" → `{ min: 100000, max: 300000 }`
- 시스템 프롬프트 원칙 8-1에 명시
- UI는 0~100만 슬라이더(5만 단위), 상단 끝은 상한 없음으로 저장 (ADR-018)
...
8. **대화 중 프로필 단서 감지 → `updateProfile` 호출** — 변경 필드만 1회 호출, mode/reason 포함
   - **8-1**: budget은 사용자가 말한 금액을 그대로 min/max에 담음 (단위 원)
...
**옵션 상수**:
- `STYLE_OPTIONS` — 캐주얼/미니멀/스트릿/빈티지/베이직/스포티/아메카지/클래식/고프코어
- `POPULAR_BRANDS` — 무신사 스탠다드/유니폼브릿지/디스이즈네버댓/커버낫/앤더슨벨/아이앱 스튜디오/아디다스
- `SIZE_OPTIONS` — XS/S/M/L/XL
- `BUDGET_MIN`·`BUDGET_MAX`·`BUDGET_STEP` — 예산 슬라이더 범위 0~100만, 5만 단위
