...
- SerpApi는 무료 크레딧이 월 250회라 채팅 요청 경로에서 검색을 실시간 호출하지 않는다. 사용자 요청에서 허용되는 호출은 `/go/[id]`의 첫 클릭 상품 페이지 찾기뿐이다(수집 후 24시간 안이면 판매처 조회, 그 외엔 구글 웹검색 최대 2회). 찾은 주소는 `direct_url`에 저장해 같은 상품은 다시 호출하지 않는다. SerpApi 계정에 카드를 등록하지 않는다
